...
class Api {
  ...
  setHeader(setHeader: boolean = true) {
    ...
    if (setHeader) {
      ...
      if (tokenUser) {
        headersConfig = { ...headersConfig, Authorization: tokenUser };
      }
      if (Token.getToken('current_branch')) {
        headersConfig = {
          ...headersConfig,
          currentbranchid: `${
            JSON.parse(Token.getToken('current_branch') as string).id
          }`,
        };
      }
    }
    ...
    return this.api
      ...
      .catch((error: any) => {
        if (error.response) {
          ...
          alert('NETWORKING_PROBLEM_DESCRIPTION');
          callback(error.response.status, error.response.data);
        }
      });
  }
